...
function init() {
  authBTAPI()
    ...
    .then(function() {
      APIState.webSocket.on('message', manageMessage(message));
...
function wsConnect() {
  ...
  console.log(APIState.webSocket);
}
...
function manageMessage(msg) {
  ...
  if (calls[responseId]) {
    if (message.type = "CALL_EVENT") {
      ...
      if (dtmf.length() > 0 && calls[responseId].callback) {
        calls[responseId].callback(dtmf);
      }
